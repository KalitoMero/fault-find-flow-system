-- Erweitere die search_excel_row Funktion um Ressource-Spalte
DROP FUNCTION IF EXISTS public.search_excel_row(text, text, text, text, text, text, text, jsonb);
...
CREATE OR REPLACE FUNCTION public.search_excel_row(
  ...
  p_resource_column text DEFAULT NULL,
...
BEGIN
  ...
  IF p_resource_column IS NOT NULL AND v_row.row_data->>p_resource_column IS NOT NULL THEN
    v_additional_data := jsonb_set(v_additional_data, '{Ressource}', to_jsonb(v_row.row_data->>p_resource_column));
  END IF;
