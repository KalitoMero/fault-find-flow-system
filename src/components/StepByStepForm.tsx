...
const StepByStepForm: React.FC<StepByStepFormProps> = ({
  ...
  const loadN8nSettings = useCallback(async () => {
    try {
      const {
        data: settings
      } = await supabase.from('n8n_settings').select('webhook_url, is_enabled').is('user_id', null).maybeSingle();
  ...
  const checkExcelData = useCallback(async (orderNumber: string, afoNumber?: string): Promise<string | null> => {
    ...
    console.log('🔍 Searching Excel database for:', {
      orderNumber,
      afoNumber
    });
    setIsSearching(true);
    ...
    try {
      ...
      const {
        data: result,
        error
      } = await supabase.rpc('search_excel_row', {
        p_order_number: orderNumber,
        p_afo_number: afoNumber,
        p_order_column: settings.orderNumberColumn,
        // Now contains column name like "bab_nr"
        p_afo_column: settings.afoNumberColumn,
        // Now contains column name like "afo_nr"
      ...
      if (result) {
        ...
        if (typedResult.department) {
          ...
          const department = departments.find(d => d.code?.trim().toUpperCase() === deptCode || d.name?.trim().toUpperCase() === deptCode);
          if (department) {
            ...
            const teamLeader = await findTeamLeaderForDepartmentOrResource(department.id, resourceName);
            ...
            if (teamLeader && teamLeader !== 'System') {
              ...
              } = await supabase.from('profiles').select('department_id, departments(name)').eq('id', teamLeader).single();
  ...
  const findTeamLeaderForDepartmentOrResource = async (departmentId: string | null, resourceName: string | null): Promise<string> => {
    try {
      console.log('🔍 Searching team leader for:', {
        resourceName,
        departmentId
      });

      // Verwende die Funktion aus resourceStorage, die Ressource priorisiert
      const teamLeaderId = await findTeamLeaderForResourceOrDepartment(resourceName, departmentId);
      if (teamLeaderId) {
  ...
  const handleNext = async () => {
    ...
    setFields(prev => prev.map((field, index) => index === currentStep ? {
      ...field,
      completed: true
    } : field));
  ...
  // Review Screen
  if (showReview) {
    return <div className="min-h-screen bg-light-blue p-6 md:p-8">
        <div className="fixed top-6 left-6 md:top-8 md:left-8 z-10">
          <Button onClick={() => setShowReview(false)} variant="outline" className="flex items-center gap-2">
          ...
          <Card className="bg-white shadow-xl">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {fields.filter(f => f.type !== 'textarea').map(field => <div key={field.id} className="p-4 bg-gray-50 rounded-lg">
                ...
                {additionalExcelData.Ressource && <div className="p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center gap-2 mb-2">
              ...
              {fields.filter(f => f.type === 'textarea' && f.value).map(field => <div key={field.id} className="p-4 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    {field.icon}
              ...
              {(excelDepartmentName || Object.keys(additionalExcelData).length > 0) && <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <h4 className="font-medium text-blue-800 mb-2">
                    Excel-Daten
                  </h4>
                  <div className="space-y-1 text-sm">
                    {excelDepartmentName && <div>
                        <span className="text-blue-600">Abteilung:</span> {excelDepartmentName}
                      </div>}
                    {assignedTeamLeader !== 'System' && <div><span className="text-blue-600">Teamleiter:</span> {getTeamLeaderDisplayName(assignedTeamLeader)}</div>}
                    {additionalExcelData.Ressource && <div><span className="text-blue-600">Ressource:</span> <span className="font-semibold">{additionalExcelData.Ressource}</span></div>}
                    {Object.entries(additionalExcelData).filter(([key]) => key !== 'Ressource').map(([key, value]) => <div key={key}><span className="text-blue-600">{key}:</span> {value}</div>)}
                  </div>
                </div>}

              <div className="flex justify-center gap-3 pt-6">
                <Button onClick={() => setShowReview(false)} variant="outline" size="lg" className="px-6 py-3">
                  Bearbeiten
                </Button>
                <Button onClick={handleSubmit} disabled={isSubmitting} className="px-6 py-3 bg-green-600 hover:bg-green-700" size="lg">
                  {isSubmitting ? <>
  ...
  return <div className="min-h-screen bg-light-blue p-6 md:p-8">
      ...
      <div className="fixed top-6 left-6 md:top-8 md:left-8 z-10">
        <Button onClick={onClose} variant="outline" size="icon" className="rounded-full shadow-lg">
        ...
        <Card className="bg-white/80 backdrop-blur-sm">
          <CardContent className="p-4">
            <div className="space-y-3">
              {/* Excel Status Info */}
              {(excelDepartment || Object.keys(additionalExcelData).length > 0) && <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex justify-between items-center text-sm flex-wrap gap-2">
                    ...
                    {Object.entries(additionalExcelData).filter(([key]) => key !== 'Artikelnummer' && key !== 'Artikelbezeichnung' && key !== 'Ressource').map(([key, value]) => <div key={key}>
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                return <div key={field.id} onClick={() => isCompleted ? handleFieldClick(index) : undefined} className={`
                        flex items-center gap-2 rounded-lg transition-all duration-300 ${isCurrentField ? 'p-4 bg-blue-100 border-2 border-blue-400 shadow-lg scale-105 cursor-default' : isCompleted ? 'p-2 bg-green-50 border border-green-200 cursor-pointer hover:bg-green-100' : 'p-2 bg-gray-50 border border-gray-200'}
                      `}>
                      <div className={`${isCurrentField ? 'text-blue-600' : isCompleted ? 'text-green-600' : 'text-gray-400'}`}>
                        {field.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`text-xs font-medium ${isCurrentField ? 'text-blue-800' : isCompleted ? 'text-green-800' : 'text-gray-500'}`}>
                          {field.type === 'quantity' ? field.quantityType || 'Ausschussmenge' : field.label}
                        </p>
                        <p className={`text-sm truncate ${isCurrentField ? 'text-blue-700' : isCompleted ? 'text-green-600' : 'text-gray-400'}`}>
                          {field.value || (isEmpty ? 'Nicht ausgefüllt' : field.value)}
                        </p>
                      </div>
                      {isCompleted && !isCurrentField && <Edit3 className="h-3 w-3 text-green-600" />}
                    </div>;
              })}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                const isCompleted = field.completed;
                const isEmpty = !field.value;
                return <div key={field.id} onClick={() => isCompleted ? handleFieldClick(fieldIndex) : undefined} className={`
                        flex items-start gap-3 rounded-lg transition-all duration-300 ${isCurrentField ? 'p-4 bg-blue-100 border-2 border-blue-400 shadow-lg scale-[1.02] cursor-default' : isCompleted ? 'p-3 bg-green-50 border border-green-200 cursor-pointer hover:bg-green-100' : 'p-3 bg-gray-50 border border-gray-200'}
                      `}>
                      <div className={`${isCurrentField ? 'text-blue-600' : isCompleted ? 'text-green-600' : 'text-gray-400'}`}>
                        {field.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium ${isCurrentField ? 'text-blue-800' : isCompleted ? 'text-green-800' : 'text-gray-500'}`}>
                          {field.label}
                        </p>
                        <div className={`text-sm ${isCurrentField ? 'text-blue-700' : isCompleted ? 'text-green-600' : 'text-gray-400'}`}>
                          {field.value ? <p className="truncate">{field.value}</p> : <p>Nicht ausgefüllt</p>}
                        </div>
                      </div>
                      {isCompleted && !isCurrentField && <Edit3 className="h-4 w-4 text-green-600" />}
                    </div>;
              })}
              </div>
            </div>
        ...
        <Card className="bg-white shadow-xl min-h-[600px] flex flex-col justify-center">
          ...
          <CardContent className="space-y-6 flex-1 flex flex-col justify-center">
            <div className="flex flex-col items-center space-y-4">
              {currentField.type === 'textarea' ? <div className="w-full max-w-md space-y-4">
                  <div className="flex gap-2 items-start">
                    <Textarea ref={textareaRef} value={currentField.value} onChange={e => handleFieldUpdate(currentField.id, e.target.value)} onClick={() => {
                ...
                }} placeholder={currentField.placeholder} rows={8} disabled={isSearching && currentStep === 0} className="text-center text-lg flex-1 mx-0 my-0 px-px py-[40px]" />
                    {(currentField.id === 'problemDescription' || currentField.id === 'correctiveAction') && <div className="flex flex-col gap-0.5 self-start -mt-2">
                  ...
                  }} className="h-10 px-4" title="Letztes Zeichen löschen">
                          <Delete className="h-5 w-5" />
                        </Button>
                        <Button type="button" variant="outline" size="default" onClick={() => handleFieldUpdate(currentField.id, '')} title="Text löschen" className="h-10 text-right px-0">
                          <Trash2 className="h-5 w-5" />
                        </Button>
                        <div className="mt-1">
                          <AudioRecorderN8n key={currentField.id} onTranscription={(transcription, audioBlob) => {
                      handleFieldUpdate(currentField.id, transcription);
                      if (audioBlob) {
                        setAudioFiles(prev => ({
                          ...prev,
                          [currentField.id]: audioBlob
                        }));
                      }
                    }} label={`${currentField.label} aufnehmen`} webhookUrl={n8nWebhookUrl} useN8n={true} />
                        </div>
                      </div>}
                  </div>
                </div> : currentField.type === 'select' ? <div className="flex flex-col items-center space-y-4">
                  <Select value={currentField.value} onValueChange={value => handleFieldUpdate(currentField.id, value)}>
                    <SelectTrigger className="text-center text-xl max-w-md h-14">
                      <SelectValue placeholder={currentField.placeholder} />
                    </SelectTrigger>
                    <SelectContent className="bg-white border border-gray-200 shadow-lg z-50">
                      {currentField.options?.map(option => <SelectItem key={option.value} value={option.value} className="hover:bg-gray-100">
                          {option.label}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                </div> : currentField.type === 'quantity' ? <div className="flex flex-col items-center space-y-4">
                  {/* Dropdown for quantity type */}
                  <Select value={currentField.quantityType || 'Ausschussmenge'} onValueChange={value => handleQuantityTypeUpdate(currentField.id, value)}>
                  ...
                  <Input type="text" value={currentField.value} onChange={e => {
              ...
              }} placeholder={currentField.placeholder || ""} className="text-center text-xl max-w-md h-14" inputMode="numeric" autoComplete="off" disabled={isSearching && currentStep === 0} />
                  
                  {/* Touch Keypad for quantity input */}
                  <div className="w-full max-w-xs">
                    <TouchKeypad onInput={handleKeypadInput} onBackspace={handleKeypadBackspace} allowDecimal={false} />
                  </div>
                </div> : <div className="flex flex-col items-center space-y-4">
                  <Input type="text" value={currentField.value} onChange={e => handleFieldUpdate(currentField.id, e.target.value)} onKeyDown={async e => {
                if (e.key === 'Enter' && currentField.value.trim()) {
                  e.preventDefault();
              ...
              }} placeholder={currentField.placeholder} className="text-center text-xl max-w-md h-14" pattern={currentField.id === 'orderNumber' ? '[0-9.]*' : '[0-9]*'} disabled={isSearching && currentStep === 0} inputMode={currentField.id === 'orderNumber' ? 'decimal' : 'numeric'} />
                  
                  {/* Touch Keypad - Always visible for non-textarea fields */}
                  <TouchKeypad onInput={handleKeypadInput} onBackspace={handleKeypadBackspace} allowDecimal={currentField.id === 'orderNumber'} className="mt-4" />
                </div>}
            </div>


            <div className="flex justify-center gap-4 pt-6">
              {isLastStep ? <Button onClick={() => {
              setShowVirtualKeyboard(false);
              setActiveKeyboardField(null);
              if (blurTimeoutRef.current) {
                clearTimeout(blurTimeoutRef.current);
              ...
              handleSubmit();
            }} disabled={isSubmitting || !isFormComplete()} className={`px-8 py-3 text-lg transition-colors ${isFormComplete() ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-400 cursor-not-allowed'}`} size="lg">
                  {isSubmitting ? <>
                      <Settings className="h-5 w-5 mr-2 animate-spin" />
                      Wird gespeichert...
                    </> : <>
      ...
      {/* Virtual Keyboard */}
      {showVirtualKeyboard && activeKeyboardField && <VirtualKeyboard value={fields.find(f => f.id === activeKeyboardField)?.value || ''} cursorPosition={cursorPosition} onChange={(value, newCursorPos) => {
